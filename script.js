...
// La détection via GitHub API est supprimée, on se base uniquement sur site/vertions.json ou un fallback local.
async function fetchVersionsFromGitHub() {
    console.warn('fetchVersionsFromGitHub est désactivé (GitHub API non utilisé)');
    return [];
}

async function fetchVersionsFromJsonFile() {
    const candidates = ['vertions.json', 'site/vertions.json', '/site/vertions.json'];
    for (const url of candidates) {
        try {
            ...
            console.info(`fetchVersionsFromJsonFile: trouvé via ${url}`);
            return data.versions;
        } catch (error) {
            console.warn(`fetchVersionsFromJsonFile (${url}): ${error.message}`);
        }
    }
...
async function fetchVersionsFromDirectoryListing() {
    const candidates = ['vertions/', 'site/vertions/', '/site/vertions/'];

    for (const dirUrl of candidates) {
        try {
            const response = await fetch(dirUrl, { cache: 'no-cache' });
            if (!response.ok) {
                continue;
            }
            ...
            const base = normalizeUrlPath(dirUrl);

            for (const href of links) {
                const clean = href.trim();
                if (clean.endsWith('/')) {
                    // dossier
                    const folderName = clean.replace(/\/$/, '');
            ...
            for (const href of links) {
                ...
                const fileUrl = clean;
                const ext = fileUrl.includes('.') ? fileUrl.substring(fileUrl.lastIndexOf('.')).toLowerCase() : '';
                const allowed = ['.exe', '.msi', '.apk', '.dmg', '.app', '.zip', '.cab', '.tar', '.gz', '.7z'];
                if (!allowed.includes(ext)) continue;

                let folderName = '';
                if (fileUrl.indexOf('/') !== -1) {
                    folderName = fileUrl.split('/')[0];
                }

                if (folderName && folderMap.has(folderName)) {
                    folderMap.get(folderName).push(fileUrl);
                } else {
                    // direct file in root "vertions"
                    folderName = 'root';
            ...
            for (const [folderName, files] of folderMap.entries()) {
                files.forEach(file => {
                    const path = folderName === 'root' ? `${base}${file}` : `${base}${folderName}/${file}`;
                    const fileName = file.split('/').pop();
                    const size = 0;
                    versions.push({ name: folderName, file: fileName, path, size });
                });
            }

            if (versions.length > 0) {
                console.info(`fetchVersionsFromDirectoryListing: trouvé via ${dirUrl}`);
                return versions;
            }

        } catch (error) {
            console.warn(`fetchVersionsFromDirectoryListing (${dirUrl}): ${error.message}`);
        }
    }
...
async function loadVersions() {
    ...
    try {
        versions = await fetchVersionsFromJsonFile();
    } catch (err) {
        console.warn('loadVersions: erreur lors du chargement de vertions.json', err.message);
    }

    if (!Array.isArray(versions) || versions.length === 0) {
        try {
            versions = await fetchVersionsFromDirectoryListing();
        } catch (err) {
            console.warn('loadVersions: erreur lors du scan de site/vertions', err.message);
        }
    }

    if ((!Array.isArray(versions) || versions.length === 0) && typeof VERSIONS_DATA !== 'undefined' && Array.isArray(VERSIONS_DATA.versions)) {
...
function setupDownloadButtons() {
    ...
    if (directDownloadBtn) {
        directDownloadBtn.addEventListener('click', function(event) {
            event.preventDefault();
            const directUrl = 'https://github.com/vouimet109-png/3M/releases/latest/download/3M.zip';
